...
bool WwiseConsoleAkSoundDataBuilder::runWwiseConsole()
{
	...
	FString BankNamesToGenerate = fillAudioBankInfoMap(audioBankInfoMap, FillAudioBankInfoKind::AssetName, platformsToBuild, languagesToBuild, &infoCache).Replace(TEXT(" "), LINE_TERMINATOR);
...

	{
		...
		if (isUsingNewAssetManagament)
		{
			...
			for (auto& eventMapEntry : akAssetDatabase.EventMap.TypeMap)
			{
				...
				{
					if (initParameters.BanksToGenerate.Num() == 0 || initParameters.BanksToGenerate.Contains(Value->GetFName().ToString()))
					{
						...
						if (Value->NeedsRebuild(platformsToBuild, languagesToBuild, &infoCache))
						{
